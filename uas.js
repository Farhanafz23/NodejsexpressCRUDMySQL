...
app.post('/api/uas',upload.single('image'),(req, res) => {
    ...
    if (!req.file) {
        ...
        const querySql = 'INSERT INTO uas (nik,nama,tanggal_lahir,alamat,agama) values (?,?,?,?,?);';
         
        // jalankan query
        koneksi.query(querySql,[ nik,nama, tanggal_lahir,alamat,agama], (err, rows, field) => {
            // error handling
            if (err) {
    ...
    const data = { ...req.body };
    const querySql = 'INSERT INTO uas (nik,nama,tanggal_lahir,alamat,agama,foto) values (?,?,?,?,?,?);';
 
// jalankan query
koneksi.query(querySql,[ nik,nama, tanggal_lahir,alamat,agama,foto], (err, rows, field) => {
    // error handling
    if (err) {
        return res.status(500).json({ message: 'Gagal insert data!', error: err });
    }
...
app.put('/api/uas/:nik', (req, res) => {
    ...
    const queryUpdate = 'UPDATE uas SET nama=?,tanggal_lahir=?,alamat=?,agama=? WHERE nik = ?';
    ...
    koneksi.query(querySearch, req.params.nik, (err, rows, field) => {
        ...
        if (rows.length) {
            ...
            koneksi.query(queryUpdate, [nama,tanggal_lahir,alamat,agama, req.params.nik], (err, rows, field) => {
                // error handling
                if (err) {
                    return res.status(500).json({ message: 'Ada kesalahan', error: err });
                }
